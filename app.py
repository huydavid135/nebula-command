...
def sync_streamlit_secrets() -> None:
    """Make Streamlit Cloud secrets available to the connector layer via os.environ."""
    try:
...
:root{
  --bg:#030611; --panel:#080d22; --line:rgba(74,113,255,.28); --cyan:#20e7ff; --violet:#9a56ff; --pink:#ff47d6; --orange:#ff7e1d; --green:#23ffac; --text:#f5fbff; --muted:#96a8c8; --shadow:0 0 28px rgba(32,231,255,.13),0 0 60px rgba(154,86,255,.11),0 22px 55px rgba(0,0,0,.34);
}
...
.glass-card{border:1px solid rgba(69,105,231,.24);border-radius:24px;background:linear-gradient(180deg,rgba(6,11,29,.96),rgba(6,10,24,.86));box-shadow:var(--shadow);overflow:hidden}.card-head{font-family:'Orbitron';font-weight:900;font-size:1.05rem;padding:15px 18px 4px}.card-sub{color:#7f90b1;font-size:.84rem;padding:0 18px 4px}.insight{display:flex;gap:14px;padding:16px;border-bottom:1px solid rgba(69,105,231,.16)}.ins-icon{width:40px;height:40px;border-radius:13px;display:flex;align-items:center;justify-content:center}.ins-text{font-weight:800;line-height:1.38;font-size:.91rem}.timeline{display:flex;align-items:center;gap:10px;padding:12px 16px}.timeline-dot{width:10px;height:10px;background:#20e7ff;border-radius:50%;box-shadow:0 0 10px #20e7ff}.timeline-time{color:#28f6ff;font-weight:800;min-width:47px}.success{margin-left:auto;color:#22ffad;font-weight:800}.scenario-box{display:flex;gap:12px;padding:18px;height:270px;align-items:center}.scenario-tile{flex:1;border-radius:22px;padding:22px 10px;text-align:center;border:1px solid rgba(55,230,190,.26);background:linear-gradient(180deg,rgba(5,42,33,.92),rgba(7,15,31,.85))}.scenario-tile.mid{transform:translateY(-20px);border-color:rgba(50,150,255,.35);background:linear-gradient(180deg,rgba(9,26,62,.92),rgba(7,15,31,.85))}.scenario-tile.bear{border-color:rgba(255,68,116,.28);background:linear-gradient(180deg,rgba(60,13,37,.92),rgba(18,12,30,.85))}.scenario-icon{font-size:2rem}.scenario-title{font-family:'Orbitron';font-weight:900}.scenario-sub{color:#a5b4d1;font-size:.82rem}.prompt-bar{height:66px;margin:13px 0;border-radius:999px;border:1px solid rgba(130,75,255,.35);background:linear-gradient(180deg,rgba(10,12,31,.96),rgba(10,12,31,.85));display:flex;align-items:center;justify-content:space-between;padding:0 22px;box-shadow:var(--shadow)}.ask{display:flex;align-items:center;gap:12px;color:#b7c3dc;font-size:1.16rem}.mic{width:76px;height:76px;margin-top:-25px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at 40% 35%,#efb6ff,#7d48ff 45%,#1a0d44 75%);border:2px solid rgba(230,238,255,.6);box-shadow:0 0 36px rgba(169,85,247,.5);font-size:1.8rem}.prompt-icons{font-size:1.4rem;letter-spacing:14px}.api-warning{padding:18px;border:1px solid rgba(255,107,122,.35);border-radius:20px;background:rgba(255,107,122,.08);color:#ffd6dc;font-weight:700;margin:16px 0}.small-note{color:#93a7ca;font-size:.84rem}
div[data-testid="stPlotlyChart"]>div{border-radius:20px;overflow:hidden}@media(max-width:1300px){.metric-row{grid-template-columns:repeat(2,1fr)}.hero-card:after{display:none}}
</style>
...
# Live data only. SoDEX/SoSoValue first; Binance/CoinGecko fallback inside connector.
assets = ASSETS
...
with left:
    ...
    status_line = "All data sources operational" if live_count else "API connection required"
...
with main:
    # Top market bar from real rows only.
    ticker_html = "<div class='top-bar'><div class='top-left'><div class='pill-top live'><span class='dot'></span>Live Market</div>"
    if has_live:
        for row in df.head(3).itertuples():
            ticker_html += f"<div class='pill-top ticker'>✦&nbsp; {row.asset}&nbsp;&nbsp; {fmt_price(float(row.price))}<span class='up'>score {float(row.command_score):.1f}</span></div>"
    else:
        ticker_html += "<div class='pill-top ticker'>No live market data</div>"
    ticker_html += "</div><div class='top-right'><div class='pill-top search'>🔎&nbsp; Search assets, narratives, signals...</div><div class='circle-top'>💬<span class='badge'>2</span></div><div class='circle-top'>🔔<span class='badge'>1</span></div><div class='pill-top profile'><div class='avatar'></div><div><div class='profile-name'>Commander</div><div class='profile-sub'>Alpha Mode</div></div><div>⌄</div></div></div></div>"
    ...
    with mc[0]:
        metric("Mission Status", plan.stance if plan else "N/A", "Live data loaded")
    with mc[1]:
    ...
    with l2:
        ...
        fig2.update_layout(height=265, margin=dict(l=0, r=0, t=0, b=0), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="white", coloraxis_showscale=False)
    ...
    with st.expander("Live API Console"):
        st.dataframe(pd.DataFrame([h.__dict__ for h in health]), use_container_width=True, hide_index=True)
